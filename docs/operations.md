...
`beacon-ui-grid-kit` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for grid scenarios with seeded input scenarios, deterministic summary checks, and fixture-scale datasets.
...
The score is deterministic. The current threshold is 173. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 5.
